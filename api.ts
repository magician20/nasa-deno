...
router.get("/planets", (ctx) => {
  // ctx.throw(400,"Sorry planets aren't avilable!");
...
router.post("/launches", async (ctx) => {
  if (ctx.request.hasBody) {
    ...
    if (result.type === "json") {
      const value = await result.value; // an object of parsed
...
export default router; //default it's the only thing in our module
